...
<nav class="col-span-2" style="display: {showLoggedIn}">
  <div class="fixed pl-4 pr-5 h-full w-[17%]">
      ...
      <li>
        ...
        {:else}
          <a
            href="/announcements"
            on:click={handlePageChange}
            class="flex items-center gap-2 px-5 py-2 bg-zinc-900 rounded-xl"
            ><svg
              xmlns="http://www.w3.org/2000/svg"
              width="18"
              height="18"
    ...
    <div
      ...
      <a href="/"
        ><img src="./logo-white.svg" class="h-10 mb-5" alt="logo" /></a>
      <button
        on:click={logOutButton}
        class=" text-white px-5 py-2 h-10 rounded-lg outline-[0.5px] outline-stone-900 bg-zinc-700 hover:bg-[#ff8989] transition-colors"
        >Log Out</button>
      <button
        on:click={toggleDarkMode}
        class="  px-3 py-2 h-10 rounded-lg outline-[0.5px] outline-stone-900 bg-zinc-700 transition-colors">
        {#if theme == "light"}
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="18"
